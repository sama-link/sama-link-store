// Sama Link · /store/orders/track — public order tracking lookup.
//
// Looks up a single order by id-or-display_id AND email. Both fields are
// required; an email mismatch returns the same 404 as a missing order so
// callers cannot enumerate which orders exist by id alone.
//
// Returns a deliberately minimal projection — id, display_id, status,
// payment_status, fulfillment_status, created_at, total, currency_code,
// and a per-item id/title/quantity/thumbnail. Customer name, addresses,
// phone, payment cards, and any sensitive metadata are intentionally NOT
// exposed.
