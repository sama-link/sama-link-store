...
- [x] **TEST-1**: Automated coverage for the auth/cart seam — server-action tests for `loginAction` / `registerAction` / `logoutAction`, plus a contract-level test for `transferCartToCustomer` exercising the three guard paths (no cart / valid cart / bogus cart), plus cookie-pair attribute parity tests for both `setAuthCookie`/`clearAuthCookie` and `setCartId`/`clearCartIdCookie`. Branch `feature/test-1-auth-cart-seam` (cut from `develop` 2026-04-19, post-TEST-0 merge `98e0dd5`). Target Advanced Executor. **Guardrail (Tech Lead, 2026-04-19):** if production source under `apps/storefront/**` would need to change for testability, STOP and escalate — do NOT refactor production code inside TEST-1. Done 2026-04-19 (PR #6 → merge commit `1189361`). Full brief below.
