...
│  Next.js (App Router)│     │  Next.js or Medusa Admin│
│  Vercel              │     │  Node hosting           │
└──────────┬───────────┘     └────────────┬────────────┘
           │                              │
...
- RTL/LTR layout support (Arabic/English)
...
**Key directories** (to be scaffolded in Phase 1):
```
apps/storefront/
...
- **Decision deferred to Phase 6** — documented in DECISIONS.md
...
| App | Target | Strategy |
|---|---|---|
| storefront | Vercel | Automatic builds on main branch |
...
- Medusa v2 as commerce backend (not re-inventing the wheel)
- Monorepo with Turborepo for shared build pipeline
- Next.js App Router for storefront (server-first, SEO-optimal)
- PostgreSQL as single source of truth
- Shared `packages/types` to avoid type drift between apps
- Admin deferred to Phase 6 — Medusa Admin UI as default option
