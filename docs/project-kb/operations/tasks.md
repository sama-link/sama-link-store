...
- **Admin UI fix (2026-04-12):** ✅ `Dockerfile.dev` updated — `RUN npx medusa build` added at image build time; CMD changed to `cd /app/.medusa/server && npx medusa start`. Resolves blank admin page caused by `medusa develop` Vite dev server running on an unexposed random port inside Docker. Admin now served as static files at `localhost:9000/app`. Rebuild required: `docker compose -f docker-compose.dev.yml up -d --build backend`.
- **Admin session fix (2026-04-12):** ✅ `docker-compose.dev.yml` backend environment block updated — `NODE_ENV: "development"` added. Resolves session cookie `secure: true` flag set by `medusa start` CLI when `NODE_ENV` was unset (it defaulted to `production`). Cookies with `secure: true` are silently dropped over HTTP, causing `/admin/users/me` to return 401 after every successful login. Fix confirmed: `NODE_ENV=development` in running container.
- **Publishable API key runtime fix (2026-04-12):** ✅ `.env.local` had malformed value `pk_<pk_ce10e...>` (angle brackets + double `pk_` prefix). Fixed to bare `pk_ce10e11305f2a15cd26d680675ac176f9aa5227da53e988bbdf4364b2d7cdac4`. Additionally, the "sama" publishable key and 2 of 3 seeded products had no sales channel association — linked all to Default Sales Channel via direct DB inserts. All 3 products now returned by store API with the corrected key.
- **Hero CTA fix (2026-04-12):** ✅ `apps/storefront/app/[locale]/(storefront)/page.tsx` — dead `<Button type="button">` replaced with `<a href="#featured">` scroll-anchor; featured products section got `id="featured"`. Unused `Button` import removed. `tsc --noEmit` exit 0.
- **i18n debt (CAT-1):** `"Variants"` heading on product detail page (`products/[handle]/page.tsx` line 78) is hardcoded English. i18n key deferred to Phase 7 i18n completion.
