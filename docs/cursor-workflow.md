...
- **Claude** = architect, planner, reviewer, documentation owner
- **Cursor** = implementer, executes one task at a time from task briefs

The human acts as the bridge: copying briefs from Claude to Cursor, and feeding Cursor's output back to Claude for review.
...
### Rule
**Cursor does NOT update governance documents.** If Cursor's task introduces something that should be documented, it flags it in the Task Report. Claude handles the documentation update.
